...
console.log("---------------------------------------------------------");
const in_dir = '/home/somanos/tmp/restore/hubs';
...
restore_db = function(items) {
  ...
  for(let i in items){
    ...
    Shell.mkdir('-p', `${mfs_dir}/hub${r.dest.home_dir}`);
    c = `rsync -rav drumee.net:${r.src.home_dir} ${mfs_dir}/hub${r.dest.home_dir}`;
    ...
    // c = `mysql -u ${process.env.USER} ${r.dest.db_name} < ${in_dir}/${r.src.db_name}.sql`;
...
read_conf = function() {
  ...
  const count = _.after(remotes.length, ()=>{
    //console.log("QQQQQQ", out);
    restore_db(out);
    //Jsonfile.writeFileSync(`${in_dir}/migration.json`,out); 
    yp.end();
  });
  for (let r of remotes){
    // const sh = Shell.exec(`sudo ls /srv/db/${r.db_name}/db.opt`, { silent: true });
    // if (sh.code !== 0) {
    //   console.warn(`NO DB ${r.id}, ${r.db_name}, ${r.db_name}`);
    //   yp.query(`SELECT id, db_name FROM entity where db_name='${r.db_name}'`, 
    //     function(err, d){
    //       console.log("KKKKKKK", d);
    //     });
    // }
    yp.query(`SELECT name, id, h.id as hub_id, home_dir, ident, db_name FROM entity join hub h using(id) where db_name='${r.db_name}'`, 
      function(err, data){
        let l = data[0];
        if(!l){
          console.log(`NEW HUB  ${r.db_name} `, r);
          yp.call_proc('migrate_create_hub', Stringify(r),
            function(e, d){
            l = d[0][0];
            out[r.db_name]={
              src:{
